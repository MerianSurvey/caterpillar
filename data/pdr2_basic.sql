...
WHERE  
	f1.isprimary   -- Only use primary target
	-- Box coordinate search; or you can use the pre-defined region search
	-- e.g. pdr2.search_gama09h(f1.object_id)
	-- See the schema browser for details.
	AND boxSearch(f1.coord, 149.4, 150.8, 1.6, 2.82)
	-- You can put the magnitude cut here
	-- e.g., AND f1.g_cmodel_mag <= 22.0
	-- Rough FDFC cuts
	...
	-- Only select extended source. This is based on the flux ratio
	--	between PSF and CModel photometry. You can use all five bands.
	AND f1.r_extendedness_value > 0
	AND f1.i_extendedness_value > 0
	-- Basic photometric quality cut. 
	-- For reliable SED fitting, these are necessary for all bands.
	-- You can loosen these criteria but save the flags in the output for later too. e.g., the interpolatedcenter criteria might not be critical for the resolved dwarf galaxies.
	-- 1. Central pixel is not saturated or interpolated over.
