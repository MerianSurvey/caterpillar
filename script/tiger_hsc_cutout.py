...
def ra_dec_prefix(ra_arr, dec_arr, band, prefix, output='./'):
    """Get the output file name based on (RA, Dec)."""
    return [os.path.join(
        output, "{:s}_{:s}_{:s}_{:s}".format(
...
def prepare_catalog(cat, size, band, ra='ra', dec='dec', name=None, unit='arcsec',
    ...
    if name is None:
        ...
        name_arr = [
            os.path.join(output, "{:s}_{:s}_{:s}".format(
                prefix, str(name).strip(), band.lower())) for name in cat[name]]
...
def batch_cutout(incat, size=10, band='i', ra='ra', dec='dec', name=None, prefix=None,
                 output='./', unit='arcsec', label='deepCoadd_calexp', root=DATA_ROOT,
                 njobs=1, psf=True):
    ...
    # Get the (RA, Dec)
    input_cat = prepare_catalog(
        cat, size, band, ra=ra, dec=dec, name=name, unit=unit, prefix=prefix, output=output)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-o', '--output', dest='output', help="Output directory",
        default='./')
    parser.add_argument(
